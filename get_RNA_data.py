...
def make_str(l):
	...
	return l1
def get_bum_regions2(bump_regions, disordered_arrays_bumpli):
	new_bump_regions=[]
	for i in range(0, len(bump_regions)):
		for j in range(0, len(disordered_arrays_bumpli)):
			if bump_regions[i][0]== disordered_arrays_bumpli[j][0]:
				new_bump_regions.append(bump_regions[i])
...
bump_regions=[]
bump_regions=get_bum_regions("/DATA/Cem_Internship/RNAproject/5951_W_minus_bumps_30each.bed")
...
disordered_filename= "/DATA/Cem_Internship/RNAproject/rnafolding_array_codons_all_ENSTs.txt"
...
bump_regions= get_bum_regions2(bump_regions,disordered_arrays_bumpli)

disorder_len= get_disordered_lens(disordered_arrays_bumpli)



print("bump_regions[0]= ", bump_regions[0] )
print("len(bump_regions) =",len(bump_regions))
...
file1= open("RNA_disorderedness_arrays_not_bumps4.txt", "w+")
...
file1= open("RNA_disorderedness_arrays_bumps4.txt", "w+")
